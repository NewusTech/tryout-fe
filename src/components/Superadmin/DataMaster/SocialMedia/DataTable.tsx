...
import { SocialMediaResponse } from "@/types/interface";
...
const DataTable: React.FC<SocialMediaResponse> = ({ headers, data, currentPage, search, }) => {
    const accessToken = Cookies.get("accessToken"); // Ambil token langsung
    const axiosPrivate = useAxiosPrivate();
    const handleDelete = async (id: number) => {
        try {
            await axiosPrivate.delete(`/user/why/us/delete/${id}`, {
                headers: {
            ...
            //   alert
        } mutate(`/user/why/us/get?page=${currentPage}&limit=10&search=${search}`);;
    };
                        ...
                        {data?.length > 0 ? (
                            data.map((item, index) => (
                                <TableRow key={item.id} index={index}>
                                    ...
                                    <TableCell className=" text-primary">{item?.title ?? "-"}</TableCell>
                                    <TableCell className="text-primary text-start">{item?.link ?? "-"}</TableCell>
                                    {/*  */}
                                                ...
                                                <DropdownMenuContent className="w-fit mr-8 mt-1 bg-white">
                                                    ...
                                                    <DropdownMenuGroup>
                                                        <DropdownMenuItem onSelect={(e) => e.preventDefault()}>
                                                            <Link className="w-full" href={`/data-master/social-media/edit/${item?.id}`}>
