    ...
    return (
        <div className="Table mt-3">
            ...
            <div className="wrap-table flex-col gap-4 mt-3 flex md:hidden">
                {data?.length > 0 ? (
                    data.map((user, index) => (
                        <div key={user?.id} className="card-table text-sm p-4 rounded-2xl border border-gray-100 shadow-md bg-white">
                            <div className="wrap-konten flex flex-col gap-2">
                                <div className="flex justify-between gap-5">
                                    <div className="label w-[50%] font-medium text-primary">No</div>
                                    <div className="konten w-[50%] text-black/80 text-start">: {(currentPage - 1) * 10 + (index + 1)}</div>
                                </div>
                                <div className="flex justify-between gap-5">
                                    <div className="label w-[50%] font-medium text-primary">Nama Paket</div>
                                    <div className="konten w-[50%] text-black/80 text-start">: {user?.package_name ?? "-"}</div>
                                </div>
                                <div className="flex justify-between gap-5">
                                    <div className="label w-[50%] font-medium text-primary">Skor</div>
                                    <div className="konten w-[50%] text-black/80 text-start">: {user?.skor ?? "-"}</div>
                                </div>
                                <div className="flex justify-between gap-5">
                                    ...
                                    <div className="konten w-[50%] text-black/80 text-start">
                                        <div className="flex gap-2 items-center">
                                            :
                                            <div className={`text-center w-fit px-3 ${user.status === 1 ? "text-succes bg-succes/20 p-1 rounded-md" : "text-error bg-error/20 p-1 rounded-md"}`}>
                                                {user.status === 1 ? "Lulus" : "Tidak Lulus"}
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div className="flex gap-3 text-white mt-5">
                                <Link href={`/my-package/history/statistik/${user?.id}`} className="bg-secondary rounded-full w-full py-2 text-center text-xs">
                                    Statistik
                                </Link>
                                <Link href={`/my-package/history/discussion/${user?.id}`} className="bg-primary px-4 rounded-full w-full py-2 text-center text-xs">
                                    Pembahasan
                                </Link>
